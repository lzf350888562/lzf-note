...
AOF持久化的实时性更好, 因此已成为主流的持久化方案, 默认情况下 Redis 没有开启 AOF（append only file）方式的持久化, 可以通过 appendonly 参数开启, aof优先级比rdb高：
...
> 
> 其原理为**开辟一个子进程**对内存进行遍历转换成一系列 Redis 的操作指令, 序列化到一个新的 AOF 日志文件中. 序列化完毕后再将操作期间发生的**增量 AOF 日志**(父进程先写到AOF重写缓冲区)追加到这个新的 AOF 日志文件中, 追加完毕后就立即原子覆盖旧的 AOF 文件. (减少冗余命令)
...
根据多个哈希函数, 对元素进行哈希取值, 根据多个哈希值在bitmap中设置1. 

当需要判断一个元素是否在布隆过滤器中时, 只需要对元素进行同上多个哈希函数取值后, 查看bitmap对应的位是否全为1, 若全为1, 则表示该元素**可能**在存在; 若不全为1, 则表示该元素**必定**不存在.

> 布隆过滤器只会误判没有遇见过的元素.

布隆过滤器因为某一位二进制可能被多个元素的hash映射, 因此布隆过滤器无法直接处理删除数据的情况.

解决办法:

1.定时异步重建布隆过滤器.

2.计数Bloom Filter(记录该位被几个数据引用), 通过将布隆过滤器的bitmap替换为数据, 数组的位置被映射一次就进行加一, 删除时减一, 避免了每个删除需要重建bitmap.
...
**PSYNC**分为完整重同步与部分重同步, P表示partition.
...
**大Key**: 含有较大数据或含有大量成员、列表数的Key.  大Key会带来的问题有: 

Redis处理变慢; 内存不断变大引发OOM或达到maxmemory引发写阻塞或重要Key被逐出(LRU), 间接造成缓存穿透; Redis Cluster数据倾斜; 读请求占用服务器大量带宽影响其他服务等

**热Key**: 访问次数、显著高于其它Key, 热Key带来的问题有:

Redis处理变慢; Redis Cluster访问倾斜; 若热Key过期造成缓存击穿则会打穿数据库.

**找出大Key与热Key**

①通过Redis内置命令debug object对目标Key进行分析, 该命令能够根据传入的Key进行分析并返回大量数据, 其中serializedlength的值为该Key的序列化长度, 通过该值可判断对应Key是否符合大Key判定标准;

> Key的序列化长度并不等同于它在内存空间中的真实长度, 另外debug object在运行时其余请求将会被阻塞直到其执行完毕, 而该命令的运行的时间长短取决于传入的Key序列化长度的大小, 因此线上环境中不推荐使用该命令来分析大Key

②通过Redis官方客户端redis-cli的bigkeys参数发现大Key

以遍历的方式分析整个Redis实例中的所有Key并汇总以报告的方式返回结果, 但不可定制化 , 如分析STRING类型或是找出全部成员数量超过10的HASH Key.

③通过Redis官方客户端redis-cli的hotkeys参数发现热Key

返回所有Key的被访问次数, 不可定制化. 使用该方案的前提条件是将redis-server的maxmemory-policy参数设置为LFU。

④使用monitor命令在紧急情况时找出热Key

打印Redis中的所有请求, 包括时间信息、Client信息、命令以及Key信息. 在发生紧急情况时, 可通过短暂执行monitor命令并将输出重定向至文件, 在关闭monitor命令后通过对文件中请求进行归类分析即可找出这段时间中的热Key.

⑤使用开源工具redis-rdb-tools以定制化方式找出大Key

对Redis的RDB文件进行定制化的分析, 但由于分析RDB文件为离线工作, 因此对线上服务不会有任何影响, 这是它的最大优点但同时也是它的最大缺点, 离线分析代表着分析结果的较差时效性. 对于一个较大的RDB文件, 它的分析可能会持续很久很久.

⑥使用开源工具rdb_bigkeys工具找出大key
...
将热Key复制多份完全相同的Key到多个节点上解决单节点压力大问题. 缺点在于代码需要联动修改, 同时也带来了数据一致性挑战.

②使用读写分离架构增加从节点分摊热Key压力
