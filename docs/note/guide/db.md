...
Redis 支持更丰富的数据类型（支持更复杂的应用场景）。Redis 不仅仅支持简单的 k/v 类型的数据，同时还提供 list，set，zset，hash 等数据结构的存储。Memcached 只支持最简单的 k/v 数据类型(如果存数组要全部加载到内存再解析, rdis可直接通过下标获取)。
...
2. 遍历算法, 复杂度O(n).
3. 如果key过多, 会导致单线程的redis卡顿, 所有读写redis的其他指令都将卡顿.
