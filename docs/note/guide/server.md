...
高可用的核心在于避免单点, 减少单点故障(保证代码质量的前提下), 可通过限流、重试、熔断降级、异步、缓存、备份等策略.

### Keepalived+VIP

**Keepalive**是Linux轻量级别的高可用解决方案, 是基于VRRP(Virtual Router Redundancy Protocol, 虚拟路由器冗余协议) 协议的一款高可用软件, Keepalived有一台主服务器(master)和多台备份服务器(backup), 在主服务器和备份服务器上面部署相同的服务配置, 使用一个虚拟IP地址对外提供服务, 当主服务器出现故障时, 虚拟IP地址会自动漂移到备份服务器.
...
为了解决固定窗口计数器算法的临界值问题, 把时间以一定比例分片, TCP网络通信协议中就是采用该算法来解决网络拥堵问题.
...
将发请求的动作比作成注水到桶中, 处理请求的过程漏水. 往桶中以任意速率流入水, 以一定速率流出水. 当水超过桶流量则丢弃, 因为桶容量是不变的, 保证了整体的速率。

使用一个队列来实现即可, 满了拒绝, 定期从队列中拿请求来执行即可.

漏桶算法能强行限制数据的传输速率(漏水速度) , 所以**漏桶算法无法处理系统在短时间内突发的大流量**
...
桶里装令牌, 请求在被处理之前需要获取一个令牌, 可根据限流大小, 按照一定的速率往桶里添加令牌.  该算法**可以处理短时间大流量的场景, 但不能超过桶的大小**.
...
Spring Cloud Gateway 中自带的单机限流的早期版本就是基于 Bucket4j 实现的, 后来替换成了Resilience4j. Resilience4j 不仅提供限流,  还提供了熔断、负载保护、自动重试等保障系统高可用开箱即用的功能. 
...
可达性分析结束，仍在白色集合的对象即为 GC Roots 不可达，可以进行回收了。
