...
> znode通过悲观锁(版本号)机制对setData和delete进行限制防止并发环境下的更新问题.

### 主从架构应用

通常整个ZooKeeper的服务器集群管理着应用协作的关键数据, 而不适应用存储海量应用数据. 因此广泛运用于分布式系统中的主从架构, 如HBase主节点服务器（HMaster）负责跟踪区域服务器（HRegionServer）是否可用, 并分派区域到服务器. ZK在该架构下只要任务为**选举主节点, 跟踪有效的从节点, 维护应用元数据以及任务信息**.
...
每个集群服务器可通过创建临时节点/master尝试成为master, 并可设置节点内容为master主机信息. 如果已经有master已经创建了/master节点, slaves则无法创建/master节点, 但可通过监听/master节点获取master状态. 当master宕机时, slave将收到/master的NodeDeleted事件, 从节点因此可以再次尝试创建/master来成为master.
...
leader每产生一个事务会为其分配一个事务标识zxid, 用于确保整个集群按照leader确定的顺序接收并处理事务. 并且服务器在重新选举leader时需要交换zxid信息来获取状态同步进度.
...
接收消息的服务器收到vote信息中的zxid与自己的zxid做比较: 若vote信息中的zixd大于等于自己的zxid, 则将自己的vote信息替换为收到的vote信息重新发送. 即拥有最新zxid的服务器将赢得选举.

> 若多个服务器的zxid相同, 则选择其中sid最大的服务器赢得选举;
>
> 若重新发送时因为网络延迟超过了最大选举等待时间(200ms), 可能会导致错误选举(不可用), 需要等到下一次正确的选举

当某个服务器得到`quorum num`个投票时, 表示选举为leader; 其他服务器成为follower并尝试连接leader然后进行状态同步, 在同步完成之后才能进处理新的请求.

3.**事务提交**

follower接收到写状态请求后会转发给leader, leader**探索性**执行更新请求并将执行结果以事务的方式对状态更新进行广播. 过程类似2PC:

①leader向followers发送proposal消息;

②follower接收到消息后, 检查该proposal是否属于自己所追随的leader, 若是则响应leader一个ACK消息;

③当leader收到`querum num`个(包括自己)ACK消息后, 向followers发送commit消息通知提交.
...
一种基于消息传递的分布式一致性算法,  在Zookeeper的zab就是借鉴了Paxos
...
**RPC（Remote Procedure Call）** 即远程过程调用. 通过 RPC 可以帮助我们调用远程计算机上某个服务的方法,  这个过程就像调用本地方法一样简单,  不需要了解底层网络编程的具体细节。
