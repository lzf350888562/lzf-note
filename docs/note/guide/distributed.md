...
当某个服务器得到`quorum`个投票时, 表示选举为leader; 其他服务器成为follower并尝试连接leader然后进行状态同步, 在同步完成之后才能进处理新的请求.
...
分布式一致性算法, 现在流行的组件 Etcd、Consul、Nacos、RocketMQ、Redis Sentinel 底层都是采用Raft算法来确认集群中的主节点, 再通过主节点向其他节点下发指令。
