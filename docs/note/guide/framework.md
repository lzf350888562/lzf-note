...
原理是，使用 `CGLIB` 创建目标对象的代理对象，当调用目标方法时，进入拦截器方法，比如调用 `a.getB().getName()` ，拦截器 `invoke()` 方法发现 `a.getB()` 是 null 值，那么就会单独发送事先保存好的查询关联 B 对象的 sql，把 B 查询上来，然后调用 a.setB(b)，于是 a 的对象 b 属性就有值了，接着完成 `a.getB().getName()` 方法的调用。

### 缓存

一级缓存：指的是Mybatis中SqlSession对象的缓存,  执行查询以后,  查询的结果会同时存入到一个Map,  再次查询同样的数据,  先去sqlsession中查询是否有.  当调用SqlSession的修改、添加、删除、commit(),close()等方法时，就会清空一级缓存。

二级缓存：指的是Mybatis中SqlSessionFactory对象的缓存，**由同一个SqlSessionFactory对象创建的SqlSession共享其缓存**，但是其中**缓存的是数据而不是对象**,  通过存储的数据构造对象返回. 
...
可以将 `Token` 保存在 Cookie 或者 localStorage 里面,  以后客户端发出的所有请求都会携带这个令牌, 通常放在 HTTP Header 的 Authorization 字段中：`Authorization: Bearer Token`。
...
因为不允许改变JWT, 所以需要引入三方存储记录Token状态, 如过期时间.  该方案中, 通过JWT中用户唯一标识+客户端环境特征生成Redis Key, 不仅减少了key的大小可以避免人为盗取,  并使用Redis过期时间来充当Token的有效时间(没有使用JWT Payload设置过期时间, 因为不允许改变Token).

![image-20211206213233959](picture/image-20211206213233959.png)

在实际应用中, 该方式有多种变体, 可根据实际需求进行跳转.

 比如Ruoyi采用仅将为用户生成随机id存入JWT Payload中, 并以该id位key, 用户信息( 包括过期时间 ) 为value存入Redis, 每次请求在Filter获取JWT中的id, 再根据id从Redis获取用户信息, 手动判断是否过期并更新.

**允许前端改变Token令牌下实现续签**

因为允许前端改变JWT, 所以可将过期时间信息存入JWT.  为了对续签进行时间限定, 通常使用OAuth2.0标准, 即access_token+refresh_token方式, 其中refresh_token的过期时间必须要比access_token过期时间长.

1. 登录后30分钟内请求

![image-20211206213957833](picture/image-20211206213957833.png)

2. 30-60分钟中间访问, access_token过期, 使用refresh_token在认证中心刷新JWT , 认证中心根据payload重新生成两个token,前端接收到后进行替换

![image-20211206214320228](picture/image-20211206214320228.png)

3. 60分钟之后访问直接认证失败.

使用中会存在重复生成JWT的问题, 两个access_token都过期且refresh_token都没过期的请求同时访问,会生成两组不同的JWT返回给前端, 后者覆盖前者, 存在逻辑问题 , 但不影响使用.

>  没有在后台保存jwt相关信息 , 所以jwt保持了无状态特性.  但是客户端需要大量针对jwt续签的改造工作

## SSO

SSO(Single Sign On)即单点登录说的是用户登陆多个子系统的其中一个就有权访问与其相关的其他系统。

举个例子:在登陆了京东金融之后，我们同时也成功登陆京东的京东超市、京东国际、京东生鲜等子系统。
...
> 工作实践: 资源服务器可将能用户登录信息缓存到本地(session),  key为认证中心返回的token, 这样请求过来先访问本地, 本地不存在再访问认证服务器,  这样可以避免过多的网络调用.
...
OAuth 2.0标准化了AccessToken的请求与响应部分, 便于各公司接入该标准的三方授权.
