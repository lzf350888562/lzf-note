...
因为MySQL Proxy对读写分离支持很好, 但对分库分表支持较差, 可采用其多个对一主多从的集群进行读写分离代理(可采用MGR等方案),  每个集群的MySQL Proxy作为一个分片, 再上一次添加ShardingJDBC对分片进行代理, 根据指定分片规则路由到不同分片.

PS: 也可以仅使用ShardingJDBC对读写分离和分库分表一气呵成
...
在环上逻辑分配若干虚拟节点, 并建立虚拟节点与真实节点的**映射表**, 每个虚拟节点映射一个真实节点. **虚拟节点越多, 真实节点数据分布越均匀**.
...
区别在于in以子查询为驱动表, 适用于内表为小表的情况; exists以外层表为驱动表, 适用于外表小的情况.

- 默认会对group by的所有字段进行排序, 若要避免消耗, 可使用order by null禁止排序.

- 深度分页limit m,n效率低, 需要加载前面m+n项然后丢弃前面m项, 最好可以利用辅助索引将id筛选出来回表.

- 使用IN替换OR, 因为mysql对in做了相应的优化.

- 避免使用子查询, 虽然5.6使用join对子查询进行了优化, 但还要根据优化器的能力.
