...
增加actuator依赖后启动项目,控制台会输出一批端点定义,这些端点并非我们自己在程序中创建，而是由`spring-boot-starter-actuator`模块根据应用依赖和配置自动创建出来的监控和管理端点。
...
之后通过`/actuator/`可列出所有可以查看的指标, 如`/actuator/beans`
...
原生的actuator太复杂, Spring Boot Admin是一款基于Actuator的开源软件：https://github.com/codecentric/spring-boot-admin，以图形化界面的方式展示actuator监控信息, 还可以用来监控微服务系统中所有服务实例( novel-cloud项目)
...
原生actuator无法被Prometheus识别, 需要对监控指标进行转换, 引入:
...
management:
  ...
  endpoint:
    prometheus:
      enabled: true                #使用prometheus对监控数据进行转换
    health:
      show-details: always        #查看更多细节
  metrics:
    export:
      prometheus:
        enabled: true             #允许指标信息导出
```

访问`/actuator/`可发现多了`/actuator/prometheus`项指标信息, 其本质是对其他endpoint进行转换后的结果.

最后设置注册到Prometheus时的应用名字, 注入bean:
...
创建文件/etc/prometheus/prometheus.yml:
...
#以下内容为监控的应用配置
  - job_name: 'springboot-prometheus'
    scrape_interval: 5s 
    metrices_path: '/actuator/prometheus'
    static_configs:
      - targets: ['192.168.31.105:80']
```

然后使用docker启动:

```
docker run -d --name=prometheus -v /etc/prometheus/prometheus.yml:/etc/prometheus.yml -p 9090:9090  bitnami/prometheus:latest
```

之后通过9090即可访问Prometheus管理界面, 通过targets可看到添加的应用目标信息.

为了让Prometheus的数据能更友好的可视化展示, 还需要配置Grafana:

```
docker run -d --name=grafana -p 3000:3000  grafana/grafana
```

之后通过3000, 默认admin/admin即可进入Grafana界面. 为了配合Prometheus实时监控, 通过Configuration/Data sources 选择Prometheus数据源, 在url中输入Prometheus的地址并保存即可, 若添加成功, 将提示`Data source is working`; 然后通过Create/Import为数据源导入监控面板(Prometheus提供了监控面板仓库). 

在监控界面, 可以选择监控的应用等, 并且右上角可以设置Grafana向Prometheus获取监控数据的时间间隔.
