...
1.如果在postProcessBeforeInstantiation方法中如果返回非null(如使用CGLIB动态代理生成了一个对象并返回),则接下来会跳过postProcessAfterInstantiation，postProcessPropertyValues以及自定义的初始化方法(start方法), 直接执行postProcessAfterInitialization方法,并且postProcessAfterInitialization如果返回不为null,则mbd.beforeInstantiationResolved被标注为true而直接返回给ioc容器了;否则才会按照原本的流程继续执行doCreateBean方法.
